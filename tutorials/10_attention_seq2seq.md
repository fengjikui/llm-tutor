...
- 新增概念：attention score、attention weight、context vector、cross-attention 的 Q/K/V 直觉。
- 实验要验证：decoder 生成每个目标 token 时，可以动态回看 encoder 的所有位置。
- 实验不验证：它还不是 Transformer，也不是高质量翻译系统。
- 跑完重点看：attention 权重的 shape、每一行权重是否近似求和为 1，以及目标 token 更关注哪些源 token。
...
| 名称 | shape | 含义 |
|---|---|---|
| `attention_weights` | `[batch, target_len, source_len]` | 当前目标位置在看源句哪里 |
| `context` | `[batch, target_len, hidden]` | 从源句按权重取回的信息 |
| `logits` | `[batch, target_len, vocab_size]` | 最终预测每个目标 token 的词表分数 |

attention weight 不是最终答案，它只是“取信息的比例”。logits 才会进入 cross entropy。
...
从下一章开始，最大变化是 Q/K/V 的来源：

| 类型 | Query 来自 | Key/Value 来自 | 典型用途 |
|---|---|---|---|
| Cross-attention | decoder 当前状态 | encoder outputs | 翻译 decoder 回看源句 |
| Self-attention | 同一个序列 | 同一个序列 | Transformer/GPT 内部 token 互相读取 |

所以 self-attention 不是全新魔法，而是把“查询并取回信息”这件事放回同一段序列内部。
