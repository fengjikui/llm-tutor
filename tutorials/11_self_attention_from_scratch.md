...
- 新增概念：self-attention、Q/K/V、scaled dot-product、multi-head、padding mask、causal mask。
- 实验要验证：同一段序列内部的 token 可以互相查询，并且 mask 能控制哪些位置允许被看见。
- 实验不验证：它还不是完整 Transformer block，也没有训练一个文本模型。
- 跑完重点看：attention 权重 shape、padding 位置权重是否被压掉、causal mask 下未来 token 权重是否为 0。
...
| 类型 | Q 的长度 | K/V 的长度 | Q/K/V 来源 |
|---|---:|---:|---|
| Cross-attention | `target_len` | `source_len` | Q 来自 decoder，K/V 来自 encoder |
| Self-attention | `seq_len` | `seq_len` | Q/K/V 都来自同一段序列 |

所以 cross-attention 里 `query_len` 和 `key_len` 可以不同；self-attention 里通常相同，都是当前序列长度。
...
不过，“不同 head 学不同语法关系”只是一个帮助理解的直觉，不是保证。真实模型里有些 head 可能很重要，有些可能冗余，也有些模式很难解释。本章实验只检查 shape 和 mask，不声称每个 head 都一定学到清晰的人类概念。
...
它和标签错位要一起理解。第 `t` 个输入位置的 hidden state 可以看见 `x[0..t]`，但它要预测的是 `y[t]`，也就是原始文本里的下一个 token：
...
所以 causal mask 不是让模型“只能看一个 token”，而是让每个位置只能看自己和左边历史，不能偷看右边答案。
